import { BANDS, CONTENT_VERSION, validBoard, isSolved, clone } from './engine.js';
...
export function validateStore(value, puzzles) {
  ...
  for (const profile of value.profiles) {
    ...
    for (const [id,a] of Object.entries(profile.attempts)) {
      const p = byId.get(id);
      if (!p || !a || a.revision !== (p.revision || 1) || !validBoard(p,a.board) || !Array.isArray(a.history) || a.history.length > 120 || !Number.isSafeInteger(a.moves) || a.moves < 0 || !Number.isInteger(a.hintLevel) || a.hintLevel < 0 || a.hintLevel > 3 || typeof a.helpUsed !== 'boolean' || typeof a.completed !== 'boolean' || !Number.isFinite(a.lastPlayed) || a.lastPlayed < 0 || (isSolved(p,a.board) && !a.completed)) throw new Error(`Saved puzzle ${id} does not match this puzzle pack.`);
      if (a.history.length > Math.min(a.moves,120)) throw new Error(`Undo history for ${id} is not valid.`);
      for (const [i,h] of a.history.entries()) if (!h || !validBoard(p,h.board) || h.moves !== a.moves-a.history.length+i) throw new Error(`Undo history for ${id} is not valid.`);
    }
  }
  ...
  return { schemaVersion: 1, contentVersion: CONTENT_VERSION, activeProfileId: value.activeProfileId, profiles: value.profiles.map(p => ({ id:p.id,name:p.name,band:p.band,avatar:p.avatar,sound:p.sound,attempts:Object.fromEntries(Object.entries(p.attempts).map(([id,a]) => [id,{revision:a.revision,board:clone(a.board),history:clone(a.history),moves:a.moves,hintLevel:a.hintLevel,helpUsed:a.helpUsed,completed:a.completed,lastPlayed:a.lastPlayed}])),...(Object.hasOwn(p,'journey') ? {journey:validateJourney(p.journey,p,puzzles)} : {}) })) };
}
